# 四足站立策略真机部署
# standing1 policy: 30维 obs -> 8维 action
# 观测顺序: base_ang_vel(3), projected_gravity(3), joint_pos_rel(8), joint_vel_rel(8), last_action(8)
# 动作: 8个关节位置残差

# D:\Conda\envs\env_isaaclab\python.exe -B 4_quad_leg_standing_1.py --default_mode motor_zero --active_joints rr --output_scale 1 --target_rate_limit_deg_s 10 --default_rate_limit_deg_s 3 --default_reached_threshold_deg 1.0 --default_reached_hold_s 0.5 --default_timeout_s 15 --warmup_s 1 --tau_limit 8
# python 4_quad_leg_standing_1.py --default_mode motor_zero --active_joints rr --output_scale 1 --target_rate_limit_deg_s 10 --default_rate_limit_deg_s 3 --default_reached_threshold_deg 1.0 --default_reached_hold_s 0.5 --default_timeout_s 15 --warmup_s 1 --tau_limit 8
...
DEFAULT_POLICY = (
    r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_standing_1\2026-06-29_20-24-28\exported\policy.pt"
)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ============================================================
        parser = argparse.ArgumentParser(description="Bennett quad_leg_standing1 policy sim2real runner.")
        ...
        parser.add_argument("--output_scale", type=float, default=0.20, help="Extra safety scale on policy residual action.")
        parser.add_argument("--action_scale_deg", type=float, default=None, help="Action scale. Defaults to env.yaml joint_pos scale.")
        parser.add_argument("--target_rate_limit_deg_s", type=float, default=20.0, help="Residual target rate limit in deg/s.")
        ...
        parser.add_argument("--tau_limit", type=float, default=8.0, help="Runtime safety torque stop threshold in Nm.")
        ...
        if args.action_scale_deg is None:
            action_scale_rad = train_align.get("action_scale_rad", 0.20)
        ...
        try:
            ...
            raise RuntimeError("damiao_6.py requires torch in the active Python environment.") from exc
        ...
        print(f"[POLICY-IN-DAMIAO-6] loaded: {policy_path}")
        ...
        print("[OBS] order = base_ang_vel(3), projected_gravity(3), joint_pos_rel(8), joint_vel_rel(8), last_action(8)")
        ...
        if args.dry_run:
            ...
            while running.is_set():
                ...
                if now >= next_policy_time:
                    ...
                    obs = torch.tensor(
                        base_ang_vel_obs + projected_gravity_obs + [0.0] * 8 + [0.0] * 8 + last_action.tolist(),
                        dtype=torch.float32,
                    ).unsqueeze(0)
                    with torch.no_grad():
        ...
        while running.is_set():
            ...
            if startup_phase == "ramp_default":
                ...
                if warmup_start_time is not None and now - warmup_start_time >= args.warmup_s:
                    ...
                    print("[SAFE-START] policy enabled after default-pose gate.", file=sys.stderr)
                ...
                obs = torch.tensor(
                    base_ang_vel_obs + projected_gravity_obs + joint_pos_obs + joint_vel_obs + last_action.tolist(),
                    dtype=torch.float32,
                ).unsqueeze(0)
                with torch.no_grad():
                    action = policy(obs).squeeze(0).to(torch.float32).clamp(-1.0, 1.0)
