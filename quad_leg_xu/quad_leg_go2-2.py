# 四足 go2-2 策略真机部署
# quad_leg_go2-2 policy: 33维 obs -> 8维 action
...
# 先 dry-run 检查 33维 obs / IMU / policy 输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-2.py --keyboard --duration_s 10
# 首次上电建议只跑 RR 小输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-2.py --keyboard --output_scale 0.05 --default_mode motor_zero --active_joints rr --default_rate_limit_deg_s 3 --default_reached_threshold_deg 1.0 --default_reached_hold_s 0.5 --default_timeout_s 15 --warmup_s 1 --tau_limit 6
...
DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-2\flat\2026-07-09_17-18-35\exported\policy.pt"
# If you trained rough, pass --policy E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-2\rough
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ============================================================
        parser = argparse.ArgumentParser(description="Bennett quad_leg_go2-2 policy sim2real runner.")
        ...
        parser.add_argument("--keyboard_min_x", type=float, default=-1, help="Minimum keyboard command_x in m/s.")
        parser.add_argument("--keyboard_max_x", type=float, default=1, help="Maximum keyboard command_x in m/s.")
        parser.add_argument("--command_deadband", type=float, default=COMMAND_DEADBAND_DEFAULT, help="Reserved for compatibility; go2-2 observes raw velocity command.")
        ...
        try:
            ...
            raise RuntimeError(r"quad_leg_xu\quad_leg_go2-2.py requires torch in the active Python environment.") from exc
        ...
        def make_policy_obs(base_ang_vel_obs, projected_gravity_obs, joint_pos_obs, joint_vel_obs, last_action_tensor, phase_elapsed_s: float):
            vel_cmd = [float(args.command_x), 0.0, 0.0]
            obs_values = (
                base_ang_vel_obs
            ...
            if len(obs_values) != 33:
                raise RuntimeError(f"go2-2 observation length mismatch: {len(obs_values)} != 33")
            return torch.tensor(obs_values, dtype=torch.float32).unsqueeze(0), 0.0, [1.0, 1.0, 1.0, 1.0]
        print(f"[POLICY-IN-DAMIAO-go2-2] loaded: {policy_path}")
        ...
        print(f"[GO2-2] command_x={args.command_x:.3f}({command_x_source}); raw velocity command observation, no gait phase terms")
        ...
        print("[OBS] order = base_ang_vel(3), projected_gravity(3), velocity_command(3), joint_pos_rel(8), joint_vel_rel(8), last_action(8)")
            ...
            if startup_phase == "ramp_default":
                ...
                if warmup_start_time is not None and now - warmup_start_time >= args.warmup_s:
                    if keyboard_command is not None and not keyboard_command.armed:
                        ...
                        print("[SAFE-START] policy enabled after default-pose gate; go2-2 policy started.", file=sys.stderr)
                ...
                if keyboard_command is None or keyboard_command.armed:
                    ...
                    policy_phase_start_time = now
                    next_policy_time = now
                    print("[SAFE-START] policy ARMED by keyboard; go2-2 policy started.", file=sys.stderr)
