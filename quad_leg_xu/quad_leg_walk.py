# Bennett quad_leg_walk3 policy real-robot deployment
# walk3 policy: 35-dim obs -> 8-dim action
# Observation order: base_ang_vel(3), projected_gravity(3), velocity_command(3),
# joint_pos_rel(8), joint_vel_rel(8), last_action(8), gait_phase(sin,cos)(2)
# Action: 8 joint position residuals, JointPositionAction(use_default_offset=True).
#
# Dry-run first:
...
if sys.platform == "win32":
    ...
    # This file lives in u2canfd/quad_leg_xu, while dlls lives in u2canfd/dlls.
    project_root = script_dir if (script_dir / "dlls").is_dir() else script_dir.parent
...
DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_xu\quad_leg_walk3\2026-07-08_22-56-39\exported\policy.pt"
WALK_CYCLE_TIME = 2.0
...
class KeyboardCommandX:
    """Non-blocking keyboard control for walk3 command_x and policy arming.

    walk3 was trained with vx in [-0.16, 0.16] m/s. The keyboard keeps
    command_x inside that training range and uses Home/0/X as zero command.

    Keys:
      Enter / Space: arm policy after safe-start
      PgUp / W: increase command_x
      PgDn / S: decrease command_x
      Home / 0 / X: set command_x to 0
    ...
    def _apply_key(self, key: str, extended: bool = False) -> None:
        ...
        if extended and key.upper() == "I":  # PgUp
            ...
            self.command_x = 0.0
            print("[KEYBOARD] command_x set to 0.000 m/s")
        elif key in ("\r", " "):
            if not self.armed:
                self.armed = True
...
def walk_gait_phase(elapsed_s: float, cycle_time: float = WALK_CYCLE_TIME) -> Tuple[List[float], float]:
    phase = (float(elapsed_s) / float(cycle_time)) % 1.0
    phase_rad = 2.0 * math.pi * phase
    return [math.sin(phase_rad), math.cos(phase_rad)], phase
...
if __name__ == "__main__":
    ...
    try:
        ...
        #
        # --output_scale
        #   策略动作额外缩放系数。默认 1.0，保持训练 action pipeline 等价；手动改小属于非训练等价安全测试。
        #
        # --action_scale_deg
        #   策略动作基础角度尺度。默认从 env.yaml 读取；一般不要手动改。
        #
        # --target_rate_limit_deg_s
        #   默认关闭，保持训练 action pipeline 等价。只有显式传入时才启用运行时目标角限速；启用后属于非训练等价安全测试模式。
        #
        # --kp / --kd
        ...
        # ============================================================
        parser = argparse.ArgumentParser(description="Bennett quad_leg_walk3 policy sim2real runner.")
        ...
        parser.add_argument("--target_rate_limit_deg_s", type=float, default=None, help="Residual target rate limit in deg/s. Default None disables runtime limiting and matches training.")
        parser.add_argument("--command_x", type=float, default=None, help="Forward velocity command in m/s. Defaults to 0.08 if not using keyboard.")
        parser.add_argument("--keyboard", action="store_true", help="Enable live PgUp/PgDn keyboard control for command_x.")
        parser.add_argument("--keyboard_step_x", type=float, default=0.05, help="PgUp/PgDn command_x increment in m/s.")
        parser.add_argument("--keyboard_min_x", type=float, default=-0.16, help="Minimum keyboard command_x in m/s, matching walk3 training range.")
        parser.add_argument("--keyboard_max_x", type=float, default=0.16, help="Maximum keyboard command_x in m/s, matching walk3 training range.")
        ...
        parser.add_argument("--tau_limit", type=float, default=15.0, help="Runtime safety torque stop threshold in Nm.")
        ...
        parser.add_argument("--default_rate_limit_deg_s", type=float, default=10.0, help="Rate limit used while moving from current pose to trained default pose.")
        ...
        if args.command_x is None:
            if args.keyboard:
                args.command_x = 0.0
                command_x_source = "keyboard_default_zero"
            else:
                args.command_x = train_align.get("command_x", 0.08)
        ...
        if args.target_rate_limit_deg_s is None:
            target_rate_limit_rad_s = None
            target_rate_limit_text = "disabled(strict train-equivalent)"
            target_step_text = "n/a"
        else:
            target_rate_limit_rad_s = math.radians(args.target_rate_limit_deg_s)
            target_rate_limit_text = f"{args.target_rate_limit_deg_s:.2f}deg/s(non-train-equivalent)"
            target_step_text = f"{args.target_rate_limit_deg_s / args.policy_rate_hz:.4f}deg/step"
        ...
        mit_kp = args.kp if args.kp is not None else train_align.get("stiffness", 45.0)
        ...
        # Training default joint positions from BENNETT_CFG_V1 / env.yaml, rad:
        #   FL_thigh=+0.14, FR_thigh=-0.14, RL_thigh=+0.14, RR_thigh=-0.14
        #   FL_calf=-0.28, FR_calf=-0.28, RL_calf=-0.28, RR_calf=-0.28
        # Action is JointPositionAction(use_default_offset=True), so policy output is a residual around this pose.
        ...
        try:
            ...
            raise RuntimeError("quad_leg_walk.py requires torch in the active Python environment.") from exc
        ...
        def imu_obs_terms():
            ...
            # 训练中所有观测 scale=null clip=null，原始值直送策略网络
            base_ang_vel_obs = clipped_scaled(gyro, -10.0, 10.0, 1.0)  # raw rad/s, no scaling
        ...
        def make_policy_obs(base_ang_vel_obs, projected_gravity_obs, joint_pos_obs, joint_vel_obs, last_action_tensor, phase_elapsed_s: float):
            velocity_command = [float(args.command_x), 0.0, 0.0]
            gait_phase, phase = walk_gait_phase(phase_elapsed_s, WALK_CYCLE_TIME)
            obs_values = (
                base_ang_vel_obs
            ...
            if len(obs_values) != 35:
                raise RuntimeError(f"walk3 observation length mismatch: {len(obs_values)} != 35")
            return torch.tensor(obs_values, dtype=torch.float32).unsqueeze(0), phase

        print(f"[POLICY-IN-DAMIAO-WALK3] loaded: {policy_path}")
        ...
        print(
            ...
            f"target_rate_limit={target_rate_limit_text} "
            f"target_step={target_step_text}"
        )
        print(
        ...
        print(f"[WALK3] command_x={args.command_x:.3f}({command_x_source}) command_y=0.000 yaw_rate=0.000 cycle_time={WALK_CYCLE_TIME:.3f}s")
        ...
        print("[OBS] order = base_ang_vel(3), projected_gravity(3), velocity_command(3), joint_pos_rel(8), joint_vel_rel(8), last_action(8), gait_phase(2)")
        ...
        print(
            ...
            "runtime target limiter is disabled unless --target_rate_limit_deg_s is explicitly set; "
            ...
            while running.is_set():
                ...
                if now >= next_policy_time:
                    ...
                    if keyboard_command is not None and not keyboard_command.armed:
                        ...
                        with torch.no_grad():
                            action = torch.tanh(policy(obs).squeeze(0).to(torch.float32))
                        if action.numel() != 8:
                    ...
                    if loop_count % max(1, int(args.policy_rate_hz // 2)) == 0:
                        print(
                            f"armed={keyboard_command.armed if keyboard_command is not None else True} cmd_x={args.command_x:.3f} phase={phase:.3f} "
        ...
        while running.is_set():
            ...
            if startup_phase == "ramp_default":
                ...
                if warmup_start_time is not None and now - warmup_start_time >= args.warmup_s:
                    if keyboard_command is not None and not keyboard_command.armed:
                        ...
                        print("[SAFE-START] policy enabled after default-pose gate; gait phase reset to 0.", file=sys.stderr)
                ...
                if keyboard_command is None or keyboard_command.armed:
                    ...
                    policy_phase_start_time = now
                    next_policy_time = now
                    print("[SAFE-START] policy ARMED by keyboard; gait phase reset to 0.", file=sys.stderr)
            elif now >= next_policy_time:
                gyro, rpy, grav, base_ang_vel_obs, projected_gravity_obs = imu_obs_terms()
                if keyboard_command is not None:
                    args.command_x = keyboard_command.poll()
                # 训练中观测 scale=null clip=null: raw joint_pos_rel (rad), joint_vel_rel (rad/s)
                joint_pos_obs = clipped_scaled(real_offsets.tolist(), -1.0, 1.0, 1.0)
                joint_vel_obs = clipped_scaled(real_vels.tolist(), -30.0, 30.0, 1.0)  # raw rad/s
                if policy_phase_start_time is None:
                    policy_phase_start_time = now
                obs, last_phase = make_policy_obs(
                    base_ang_vel_obs,
                    projected_gravity_obs,
                    joint_pos_obs,
                    joint_vel_obs,
                ...
                if target_rate_limit_rad_s is None:
                    applied_joint_offsets = desired_joint_offsets.clone()
                else:
            ...
            if loop_count % 200 == 0:
                ...
                print(
                    f"cmd_x={args.command_x:.3f} phase={last_phase:.3f} "
