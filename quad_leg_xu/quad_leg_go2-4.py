# 四足 go2-4 低速相位策略真机部署
# quad_leg_go2-4 policy: 50维 obs -> 8维 action
...
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-4.py --keyboard --diag_sequence --log_csv logs\go2_6_diag.csv

# 先 dry-run 检查 50维 obs / IMU / policy 输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-4.py --keyboard --duration_s 10 --dry_run
# 首次上电建议只跑 RR 小输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-4.py --keyboard --output_scale 0.05 --default_mode motor_zero --active_joints rr --default_rate_limit_deg_s 3 --default_reached_threshold_deg 1.0 --default_reached_hold_s 0.5 --default_timeout_s 15 --warmup_s 1 --tau_limit 6
...
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-4\flat\2026-07-09_17-47-04\exported\policy.pt"
...
DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-5\flat\2026-07-09_22-09-36\exported\policy.pt"
...
# If you trained rough, pass --policy E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-4\rough
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ============================================================
        parser = argparse.ArgumentParser(description="Bennett quad_leg_go2-4 low-speed gait policy sim2real runner.")
        ...
        parser.add_argument("--keyboard_min_x", type=float, default=0.0, help="Minimum keyboard command_x in m/s. Go2-4 trains standstill plus forward low speed.")
        parser.add_argument("--keyboard_max_x", type=float, default=0.18, help="Maximum keyboard command_x in m/s, matching Go2-4 low-speed range.")
        parser.add_argument("--command_deadband", type=float, default=COMMAND_DEADBAND_DEFAULT, help="Abs(command_x) below this uses go2-4 stand-still phase/contact observations.")
        ...
        try:
            ...
            raise RuntimeError(r"quad_leg_xu\quad_leg_go2-4.py requires torch in the active Python environment.") from exc
        ...
        def make_policy_obs(base_ang_vel_obs, projected_gravity_obs, joint_pos_obs, joint_vel_obs, last_action_tensor, phase_elapsed_s: float):
            ...
            if len(obs_values) != 50:
                raise RuntimeError(f"go2-4 observation length mismatch: {len(obs_values)} != 50")
            return torch.tensor(obs_values, dtype=torch.float32).unsqueeze(0), phase, desired_contacts

        print(f"[POLICY-IN-DAMIAO-go2-4] loaded: {policy_path}")
        ...
        print(
            f"[GO2-4] command_x={args.command_x:.3f}({command_x_source}) "
        ...
        if args.log_csv:
            ...
            log_csv_writer.writerow([
                ...
                "command_x", "gait_phase", "joint_name", "joint_active",
        ...
        while running.is_set():
            ...
            if startup_phase == "ramp_default":
                ...
                if warmup_start_time is not None and now - warmup_start_time >= args.warmup_s:
                    if keyboard_command is not None and not keyboard_command.armed:
                        ...
                        print("[SAFE-START] policy enabled after default-pose gate; go2-4 gait phase reset to 0.", file=sys.stderr)
                ...
                if keyboard_command is None or keyboard_command.armed:
                    ...
                    policy_phase_start_time = now
                    next_policy_time = now
                    print("[SAFE-START] policy ARMED by keyboard; go2-4 gait phase reset to 0.", file=sys.stderr)
                ...
                if args.diag_sequence and policy_phase_start_time is not None:
                    ...
                    if diag_stage_index != diag_last_stage_index:
                        ...
                        print(f"\a[DIAG] stage={diag_stage_name} command_x={args.command_x:.3f}: {diag_message}", file=sys.stderr)
            ...
            if log_csv_writer is not None and loop_count % args.log_csv_every == 0:
                for i, name in enumerate(joint_names):
                    ...
                    log_csv_writer.writerow([
                        ...
                        args.command_x, last_phase, name, int(name in active_joint_names),
            ...
            if loop_count % 200 == 0:
                ...
                print(
                    f"stage={diag_stage_name} cmd_x={args.command_x:.3f} phase={last_phase:.3f} contacts={fmt_vec(last_desired_contacts, 0)} "
