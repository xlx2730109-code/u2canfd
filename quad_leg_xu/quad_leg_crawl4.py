# 四足 crawl4 策略真机部署
# quad_leg_crawl4 policy: 50维 obs -> 8维 action
...
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_crawl4.py --keyboard --duration_s 10
# 首次上电建议只跑 RR 小输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_crawl4.py --keyboard --output_scale 0.05 --default_mode motor_zero --active_joints rr --target_rate_limit_deg_s 8 --default_rate_limit_deg_s 3 --default_reached_threshold_deg 1.0 --default_reached_hold_s 0.5 --default_timeout_s 15 --warmup_s 1 --tau_limit 6
...
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_xu\quad_leg_crawl4"
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_xu\quad_leg_crawl4"
DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_xu\quad_leg_crawl4\2026-07-09_14-54-23\exported\policy.pt"
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_xu\quad_leg_crawl4\2026-07-09_14-54-23\exported\policy.pt"
...
class KeyboardCommandX:
    """Non-blocking keyboard control for crawl4 command_x and policy arming.
...
def crawl_phase_terms(
    ...
    # Order is FL, FR, RL, RR. Offsets match quad_leg_crawl4/mdp/gait_scheduler.py.
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ============================================================
        parser = argparse.ArgumentParser(description="Bennett quad_leg_crawl4 policy sim2real runner.")
        ...
        parser.add_argument("--target_rate_limit_deg_s", type=float, default=90.0, help="Residual target rate limit in deg/s. Default is effectively unbounded to match training action output.")
        ...
        parser.add_argument("--command_deadband", type=float, default=COMMAND_DEADBAND_DEFAULT, help="Abs(command_x) below this uses crawl4 stand-still observations.")
        ...
        parser.add_argument("--kp", type=float, default=None, help="MIT position gain during policy. Defaults to env.yaml stiffness.")
        parser.add_argument("--kd", type=float, default=None, help="MIT velocity gain during policy. Defaults to env.yaml damping.")
        parser.add_argument("--startup_kp", type=float, default=20.0, help="MIT position gain used only during safe-start default-pose ramp.")
        parser.add_argument("--startup_kd", type=float, default=None, help="MIT velocity gain used only during safe-start. Defaults to policy kd.")
        ...
        parser.add_argument("--standup_s", type=float, default=15.0, help="Deprecated alias for --default_timeout_s when --default_timeout_s is omitted.")
        ...
        parser.add_argument("--default_reached_threshold_deg", type=float, default=6.0, help="Max active-joint error to trained default before policy is allowed.")
        ...
        startup_mit_kp = args.startup_kp if args.startup_kp is not None else mit_kp
        startup_mit_kd = args.startup_kd if args.startup_kd is not None else mit_kd
        ...
        try:
            ...
            raise RuntimeError("quad_leg_xu\quad_leg_crawl4.py requires torch in the active Python environment.") from exc
        ...
        def make_policy_obs(base_ang_vel_obs, projected_gravity_obs, joint_pos_obs, joint_vel_obs, last_action_tensor, phase_elapsed_s: float):
            ...
            if len(obs_values) != 50:
                raise RuntimeError(f"crawl4 observation length mismatch: {len(obs_values)} != 50")
            return torch.tensor(obs_values, dtype=torch.float32).unsqueeze(0), phase, desired_contacts

        print(f"[POLICY-IN-DAMIAO-6-crawl4] loaded: {policy_path}")
        ...
        print(
            f"[TRAIN-ALIGN] policy MIT kp={mit_kp:.3f}({kp_source}) kd={mit_kd:.3f}({kd_source}) "
            f"startup_kp={startup_mit_kp:.3f} startup_kd={startup_mit_kd:.3f} "
        ...
        while running.is_set():
            ...
            if startup_phase == "ramp_default":
                ...
                if keyboard_command is None or keyboard_command.armed:
                    ...
                    policy_phase_start_time = now
                    next_policy_time = now
                    print("[SAFE-START] policy ARMED by keyboard; crawl4 phase reset to 0.", file=sys.stderr)
            ...
            for name in active_joint_names:
                ...
                command_kp = startup_mit_kp if startup_phase != "policy" else mit_kp
                command_kd = startup_mit_kd if startup_phase != "policy" else mit_kd
                control.control_mit(motor, command_kp, command_kd, q_cmd[name], 0.0, 0.0)
