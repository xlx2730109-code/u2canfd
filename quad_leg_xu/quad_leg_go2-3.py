# 四足 go2-3 低速相位策略真机部署
# quad_leg_go2-3 policy: 50维 obs -> 8维 action
...
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-3.py --keyboard --duration_s 10 --dry_run
# 首次上电建议只跑 RR 小输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-3.py --keyboard --output_scale 0.05 --default_mode motor_zero --active_joints rr --default_rate_limit_deg_s 3 --default_reached_threshold_deg 1.0 --default_reached_hold_s 0.5 --default_timeout_s 15 --warmup_s 1 --tau_limit 6
...
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-3\flat\2026-07-09_17-47-04\exported\policy.pt"
...
# If you trained rough, pass --policy E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-3\rough
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ============================================================
        parser = argparse.ArgumentParser(description="Bennett quad_leg_go2-3 low-speed gait policy sim2real runner.")
        ...
        parser.add_argument("--target_rate_limit_deg_s", type=float, default=10000.0, help="Residual target rate limit in deg/s. Default is effectively unbounded to match training action output.")
        ...
        parser.add_argument("--keyboard_min_x", type=float, default=0.0, help="Minimum keyboard command_x in m/s. Go2-3 trains standstill plus forward low speed.")
        parser.add_argument("--keyboard_max_x", type=float, default=0.18, help="Maximum keyboard command_x in m/s, matching Go2-3 low-speed range.")
        parser.add_argument("--command_deadband", type=float, default=COMMAND_DEADBAND_DEFAULT, help="Abs(command_x) below this uses go2-3 stand-still phase/contact observations.")
        ...
        parser.add_argument("--crawl_frequency_hz", type=float, default=None, help="Go2-3 gait scheduler frequency. Defaults to env.yaml/fallback 0.65.")
        ...
        parser.add_argument("--crawl_swing_height", type=float, default=None, help="Go2-3 swing height observation. Defaults to env.yaml/fallback 0.055.")
        ...
        if args.crawl_frequency_hz is None:
            args.crawl_frequency_hz = train_align.get("crawl_frequency_hz", 0.65)
        ...
        if args.crawl_swing_height is None:
            args.crawl_swing_height = train_align.get("crawl_swing_height", 0.055)
        ...
        train_default_values = [0.12, -0.24, -0.12, -0.24, 0.12, -0.24, -0.12, -0.24]
        ...
        try:
            ...
            raise RuntimeError(r"quad_leg_xu\quad_leg_go2-3.py requires torch in the active Python environment.") from exc
        ...
        def make_policy_obs(base_ang_vel_obs, projected_gravity_obs, joint_pos_obs, joint_vel_obs, last_action_tensor, phase_elapsed_s: float):
            ...
            if len(obs_values) != 50:
                raise RuntimeError(f"go2-3 observation length mismatch: {len(obs_values)} != 50")
            return torch.tensor(obs_values, dtype=torch.float32).unsqueeze(0), phase, desired_contacts

        print(f"[POLICY-IN-DAMIAO-go2-3] loaded: {policy_path}")
        ...
        print(
            f"[GO2-3] command_x={args.command_x:.3f}({command_x_source}) "
            ...
            if startup_phase == "ramp_default":
                ...
                if warmup_start_time is not None and now - warmup_start_time >= args.warmup_s:
                    if keyboard_command is not None and not keyboard_command.armed:
                        ...
                        print("[SAFE-START] policy enabled after default-pose gate; go2-3 gait phase reset to 0.", file=sys.stderr)
                ...
                if keyboard_command is None or keyboard_command.armed:
                    ...
                    policy_phase_start_time = now
                    next_policy_time = now
                    print("[SAFE-START] policy ARMED by keyboard; go2-3 gait phase reset to 0.", file=sys.stderr)
