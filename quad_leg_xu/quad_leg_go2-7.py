# 四足 go2-7 低速全向相位策略真机部署
# quad_leg_go2-7 policy: 50维 obs -> 8维 action
...
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-7.py --keyboard

# 先 dry-run 检查 50维 obs / IMU / policy 输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-7.py --keyboard --duration_s 10 --dry_run
# 首次上电建议只跑 RR 小输出：
# D:\Conda\envs\env_isaaclab\python.exe -B quad_leg_xu\quad_leg_go2-7.py --keyboard --output_scale 0.05 --default_mode motor_zero --active_joints rr --default_rate_limit_deg_s 3 --default_reached_threshold_deg 1.0 --default_reached_hold_s 0.5 --default_timeout_s 15 --warmup_s 1 --tau_limit 6
...
DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-7\flat\2026-07-10_08-37-22\exported\policy.pt"
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-4\flat\2026-07-09_19-20-46\exported\policy.pt"
# Go2-7 has an older pre-omnidirectional export in its log directory. Require
# the exact new run path so deployment cannot silently load that old policy.
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-6\flat\2026-07-09_23-34-42\exported\policy.pt"
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-6\flat\2026-07-10_00-26-02\exported\policy.pt"
# DEFAULT_POLICY = r"E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-7\flat\2026-07-10_03-13-02\exported\policy.pt"


# If you trained rough, pass --policy E:\Project\Isaaclab\bennett_rl\logs\rsl_rl\quad_leg_go2\quad_leg_go2-7\rough
...
def _parse_init_state_joint_pos(lines: List[str]) -> Dict[str, float]:
    """Parse scene.robot.init_state.joint_pos from Isaac Lab env.yaml."""
    in_init_state = False
    init_indent = 0
    in_joint_pos = False
    joint_pos_indent = 0
    ...
    for line in lines:
        ...
        if indent <= init_indent and stripped.endswith(":"):
            if values:
                return values
            in_init_state = stripped == "init_state:"
            init_indent = indent
            in_joint_pos = False
            values = {}
            continue
        ...
        if not in_joint_pos:
            if stripped == "joint_pos:":
                in_joint_pos = True
                joint_pos_indent = indent
                values = {}
            continue

        if indent <= joint_pos_indent:
            if values:
                return values
            in_joint_pos = False
            continue
        ...
        if not raw_value or raw_value.startswith("!!python/"):
            continue
        try:
            values[name.strip()] = float(raw_value.split()[0])
        except ValueError:
            continue
...
def load_train_alignment(policy_path: Path) -> dict:
    ...
    init_joint_pos = _parse_init_state_joint_pos(lines)
    if init_joint_pos:
        info["init_joint_pos"] = init_joint_pos
    in_actions = False
...
class KeyboardCommandVelocity:
    """Non-blocking omnidirectional velocity control and policy arming.

    Keys:
      Enter / Space: arm policy after safe-start
      W / S: increase/decrease forward velocity
      A / D: increase/decrease lateral velocity
      J / L: increase/decrease yaw rate
      PgUp / PgDn: increase/decrease forward velocity
      Home / 0 / X: set all velocity commands to zero
    ...
    def __init__(
        ...
        if self.min_x > self.max_x or self.min_y > self.max_y or self.min_yaw > self.max_yaw:
            raise ValueError("keyboard command minimum must not exceed its maximum")
    ...
    def _apply_key(self, key: str, extended: bool = False) -> None:
        ...
        if extended and key.upper() == "I":  # PgUp
            self.command_x = min(self.max_x, self.command_x + self.step_x)
            self._print_command()
        elif extended and key.upper() == "Q":  # PgDn
            self.command_x = max(self.min_x, self.command_x - self.step_x)
            self._print_command()
...
def crawl_phase_terms(
    ...
    command_norm = math.sqrt(float(command_x) ** 2 + float(command_y) ** 2 + float(command_yaw) ** 2)
    moving = command_norm >= float(command_deadband)
    velocity_command = [float(command_x), float(command_y), float(command_yaw)] if moving else [0.0, 0.0, 0.0]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ============================================================
        parser = argparse.ArgumentParser(description="Bennett quad_leg_go2-7 omnidirectional gait policy sim2real runner.")
        ...
        parser.add_argument("--target_rate_limit_deg_s", type=float, default=60000.0, help="Residual target rate limit in deg/s. Default is effectively unbounded to match training action output.")
        parser.add_argument("--command_x", type=float, default=None, help="Forward velocity command in m/s. Defaults to 0.10 without keyboard.")
        parser.add_argument("--command_y", type=float, default=0.0, help="Lateral velocity command in m/s.")
        parser.add_argument("--command_yaw", type=float, default=0.0, help="Yaw-rate command in rad/s.")
        parser.add_argument("--keyboard", action="store_true", help="Enable W/S, A/D, J/L omnidirectional keyboard commands.")
        parser.add_argument("--keyboard_step_x", type=float, default=0.02, help="Forward/backward command increment in m/s.")
        parser.add_argument("--keyboard_step_y", type=float, default=0.02, help="Lateral command increment in m/s.")
        parser.add_argument("--keyboard_step_yaw", type=float, default=0.10, help="Yaw-rate command increment in rad/s.")
        parser.add_argument("--keyboard_min_x", type=float, default=-0.18, help="Minimum forward/backward command in m/s.")
        parser.add_argument("--keyboard_max_x", type=float, default=0.18, help="Maximum forward/backward command in m/s.")
        parser.add_argument("--keyboard_min_y", type=float, default=-0.10, help="Minimum lateral command in m/s.")
        parser.add_argument("--keyboard_max_y", type=float, default=0.10, help="Maximum lateral command in m/s.")
        parser.add_argument("--keyboard_min_yaw", type=float, default=-0.50, help="Minimum yaw-rate command in rad/s.")
        parser.add_argument("--keyboard_max_yaw", type=float, default=0.50, help="Maximum yaw-rate command in rad/s.")
        parser.add_argument("--command_deadband", type=float, default=COMMAND_DEADBAND_DEFAULT, help="Velocity-command norm below this uses stand-still phase/contact observations.")
        ...
        parser.add_argument("--crawl_frequency_hz", type=float, default=None, help="Go2-4 gait scheduler frequency. Defaults to env.yaml/fallback 0.45.")
        parser.add_argument("--crawl_duty_factor", type=float, default=None, help="Crawl stance duty factor. Defaults to env.yaml/fallback 0.82.")
        parser.add_argument("--crawl_swing_height", type=float, default=None, help="Go2-4 swing height observation. Defaults to env.yaml/fallback 0.045.")
        ...
        parser.add_argument("--log_csv", type=str, default=None, help="Write per-joint deployment diagnostics to this CSV path.")
        parser.add_argument("--log_csv_every", type=int, default=1, help="Write one CSV sample every N control loops.")
        parser.add_argument("--diag_sequence", action="store_true", help="Run a timed diagnostic sequence after policy is armed: air, half-load, ground, 0.04m/s, 0.08m/s.")
        parser.add_argument("--diag_walk_only", action="store_true", help="With --diag_sequence, skip air/half-load and run only ground, 0.04m/s, 0.08m/s.")
        parser.add_argument("--diag_stage_s", type=float, default=5.0, help="Seconds per diagnostic sequence stage.")
        parser.add_argument("--diag_repeat_walk", action="store_true", help="After non-walking diagnostic stages, repeat 0.04 and 0.08 m/s walk stages until stopped.")
        parser.add_argument("--gyro_signs", type=str, default="1,1,1", help="Per-axis signs for gyro x,y,z after IMU read.")
        parser.add_argument("--gyro_unit", choices=("rad_s", "deg_s"), default="rad_s", help="Unit of IMU gyro data; policy expects rad/s.")
        ...
        if args.diag_stage_s <= 0.0:
            raise ValueError("--diag_stage_s must be > 0")

        if not args.policy:
            raise ValueError("Pass the exact new Go2-7 exported policy with --policy <...\\exported\\policy.pt>.")
        ...
        if args.action_scale_deg is None:
            action_scale_rad = train_align.get("action_scale_rad", 0.18)
        ...
        if args.crawl_frequency_hz is None:
            args.crawl_frequency_hz = train_align.get("crawl_frequency_hz", 0.45)
        ...
        if args.crawl_duty_factor is None:
            args.crawl_duty_factor = train_align.get("crawl_duty_factor", 0.82)
        ...
        if args.crawl_swing_height is None:
            args.crawl_swing_height = train_align.get("crawl_swing_height", 0.045)
        ...
        mit_kp = args.kp if args.kp is not None else train_align.get("stiffness", 24.0)
        mit_kd = args.kd if args.kd is not None else train_align.get("damping", 2.5)
        ...
        fallback_train_default_values = [0.12, -0.24, -0.12, -0.24, 0.12, -0.24, -0.12, -0.24]
        fallback_train_default_by_name = dict(zip(joint_names, fallback_train_default_values))
        init_joint_pos = train_align.get("init_joint_pos")
        train_default_source = "fallback"
        if isinstance(init_joint_pos, dict):
            missing_defaults = [name for name in joint_names if name not in init_joint_pos]
            if missing_defaults:
                print(
                    "[TRAIN-ALIGN] init_state.joint_pos missing "
                    + ", ".join(missing_defaults)
                    + "; using fallback train defaults.",
                    file=sys.stderr,
                )
                train_default_by_name = fallback_train_default_by_name
            else:
                train_default_by_name = {name: float(init_joint_pos[name]) for name in joint_names}
                train_default_source = "env.yaml"
        else:
            train_default_by_name = fallback_train_default_by_name
        train_default_values = [train_default_by_name[name] for name in joint_names]
        ...
        try:
            ...
            raise RuntimeError(r"quad_leg_xu\quad_leg_go2-7.py requires torch in the active Python environment.") from exc
        ...
        def imu_obs_terms():
            ...
            if args.gyro_unit == "deg_s":
                gyro = [math.radians(v) for v in gyro]
            ...
            base_ang_vel_obs = [float(v) for v in gyro]
            projected_gravity_obs = [float(v) for v in grav]
        ...
        def make_policy_obs(base_ang_vel_obs, projected_gravity_obs, joint_pos_obs, joint_vel_obs, last_action_tensor, phase_elapsed_s: float):
            ...
            if len(obs_values) != 50:
                raise RuntimeError(f"go2-7 observation length mismatch: {len(obs_values)} != 50")
            return torch.tensor(obs_values, dtype=torch.float32).unsqueeze(0), phase, desired_contacts

        print(f"[POLICY-IN-DAMIAO-go2-7] loaded: {policy_path}")
        ...
        if args.keyboard:
            keyboard_command = KeyboardCommandVelocity(
                initial_x=args.command_x,
                initial_y=args.command_y,
                initial_yaw=args.command_yaw,
            ...
            args.command_x, args.command_y, args.command_yaw = keyboard_command.poll()

        print(
            f"[GO2-7] command=[{args.command_x:+.3f}, {args.command_y:+.3f}, {args.command_yaw:+.3f}] "
            f"(vx source={command_x_source}; vy m/s; yaw rad/s) "
        ...
        if args.diag_walk_only:
            diag_stages = [
                ("ground", 0.0, "PLACE FULLY ON GROUND"),
                ("walk_0p04", 0.04, "WALK COMMAND 0.04 m/s"),
                ("walk_0p08", 0.08, "WALK COMMAND 0.08 m/s"),
            ]
        else:
            diag_stages = [
                ("air", 0.0, "HOLD ROBOT IN AIR"),
                ("half_load", 0.0, "LOWER TO HALF LOAD"),
                ("ground", 0.0, "PLACE FULLY ON GROUND"),
                ("walk_0p04", 0.04, "WALK COMMAND 0.04 m/s"),
                ("walk_0p08", 0.08, "WALK COMMAND 0.08 m/s"),
            ]
        diag_walk_start_index = max(0, len(diag_stages) - 2)
        if args.diag_sequence:
            repeat_msg = "; walking stages repeat until stopped" if args.diag_repeat_walk else ""
            print(
                f"[DIAG] timed sequence enabled: {len(diag_stages)} stages x {args.diag_stage_s:.1f}s{repeat_msg}. "
                "Timer starts when policy is armed. CSV stage column will mark each sample."
            )
        if keyboard_command is not None:
            print(
                "[KEYBOARD] W/S forward/backward, A/D lateral, J/L yaw, PgUp/PgDn forward/backward; "
                f"limits vx=[{args.keyboard_min_x:.3f}, {args.keyboard_max_x:.3f}], "
                f"vy=[{args.keyboard_min_y:.3f}, {args.keyboard_max_y:.3f}], "
                f"yaw=[{args.keyboard_min_yaw:.3f}, {args.keyboard_max_yaw:.3f}], Home/0/X zero, "
        ...
        print(f"[TRAIN-DEFAULT] source={train_default_source} " + ", ".join(f"{name}:{math.degrees(train_default_by_name[name]):+.1f}deg" for name in joint_names))
        print(
        ...
        if args.log_csv:
            log_csv_path = Path(args.log_csv).expanduser()
            log_csv_path.parent.mkdir(parents=True, exist_ok=True)
            log_csv_file = log_csv_path.open("w", newline="", encoding="utf-8")
            log_csv_writer = csv.writer(log_csv_file)
            log_csv_writer.writerow([
                "time_monotonic", "elapsed_s", "loop_count", "startup_phase", "diag_stage",
                "command_x", "command_y", "command_yaw", "gait_phase", "joint_name", "joint_active",
                "kp", "kd", "output_scale", "policy_rate_hz", "target_rate_limit_deg_s",
                "action", "desired_offset_rad", "applied_offset_rad",
                "target_sim_rad", "target_sim_deg", "train_default_rad",
                "rel_rad", "rel_deg", "q_cmd_motor_rad", "q_real_motor_rad",
                "dq_real_motor_rad_s", "tau_nm", "motor_err",
                "base_ang_vel_x", "base_ang_vel_y", "base_ang_vel_z",
                "projected_gravity_x", "projected_gravity_y", "projected_gravity_z",
                "desired_contact",
            ])
            print(f"[LOG-CSV] writing deployment diagnostics to {log_csv_path}")
            ...
            while running.is_set():
                ...
                if now >= next_policy_time:
                    ...
                    if loop_count % max(1, int(args.policy_rate_hz // 2)) == 0:
                        print(
                            f"armed={keyboard_command.armed if keyboard_command is not None else True} cmd=[{args.command_x:+.3f}, {args.command_y:+.3f}, {args.command_yaw:+.3f}] phase={phase:.3f} contacts={fmt_vec(desired_contacts, 0)} "
        ...
        diag_stage_name = "manual"
        diag_last_stage_index = None
        last_base_ang_vel_obs = [float("nan"), float("nan"), float("nan")]
        last_projected_gravity_obs = [float("nan"), float("nan"), float("nan")]
        ...
        while running.is_set():
            ...
            if startup_phase == "ramp_default":
                ...
                if warmup_start_time is not None and now - warmup_start_time >= args.warmup_s:
                    if keyboard_command is not None and not keyboard_command.armed:
                        ...
                        print("[SAFE-START] policy enabled after default-pose gate; go2-7 gait phase reset to 0.", file=sys.stderr)
                ...
                if keyboard_command is None or keyboard_command.armed:
                    ...
                    policy_phase_start_time = now
                    next_policy_time = now
                    print("[SAFE-START] policy ARMED by keyboard; go2-7 gait phase reset to 0.", file=sys.stderr)
            elif now >= next_policy_time:
                gyro, rpy, grav, base_ang_vel_obs, projected_gravity_obs = imu_obs_terms()
                last_base_ang_vel_obs = list(base_ang_vel_obs)
                last_projected_gravity_obs = list(projected_gravity_obs)
                if args.diag_sequence and policy_phase_start_time is not None:
                    diag_elapsed = max(0.0, now - policy_phase_start_time)
                    diag_stage_index = int(diag_elapsed // args.diag_stage_s)
                    if args.diag_repeat_walk and diag_stage_index >= diag_walk_start_index:
                        walk_index = diag_walk_start_index + ((diag_stage_index - diag_walk_start_index) % 2)
                        diag_stage_name, diag_command_x, diag_message = diag_stages[walk_index]
                    elif diag_stage_index < len(diag_stages):
                        diag_stage_name, diag_command_x, diag_message = diag_stages[diag_stage_index]
                    else:
                        diag_stage_index = len(diag_stages)
                        diag_stage_name, diag_command_x, diag_message = "done", 0.0, "DIAG DONE - command_x zero"
                    args.command_x = diag_command_x
                    args.command_y = 0.0
                    args.command_yaw = 0.0
                    if diag_stage_index != diag_last_stage_index:
                        diag_last_stage_index = diag_stage_index
                        print(f"\a[DIAG] stage={diag_stage_name} command=[{args.command_x:+.3f}, 0.000, 0.000]: {diag_message}", file=sys.stderr)
                elif keyboard_command is not None:
                    args.command_x, args.command_y, args.command_yaw = keyboard_command.poll()
                else:
                    diag_stage_name = "manual"
                joint_pos_obs = [float(v) for v in real_offsets.tolist()]
                joint_vel_obs = [float(v) for v in real_vels.tolist()]
            ...
            if log_csv_writer is not None and loop_count % args.log_csv_every == 0:
                for i, name in enumerate(joint_names):
                    motor = motors[name]
                    log_csv_writer.writerow([
                        now, elapsed_s, loop_count, startup_phase, diag_stage_name,
                        args.command_x, args.command_y, args.command_yaw, last_phase, name, int(name in active_joint_names),
                        mit_kp, mit_kd, args.output_scale, args.policy_rate_hz, args.target_rate_limit_deg_s,
                        float(action[i]), float(desired_joint_offsets[i]), float(applied_joint_offsets[i]),
                        float(target_sim_pos[i]), math.degrees(float(target_sim_pos[i])), float(train_default[i]),
                        float(real_offsets[i]), math.degrees(float(real_offsets[i])), float(q_cmd[name]), float(motor.Get_Position()),
                        float(motor.Get_Velocity()), float(motor.Get_tau()), int(motor.Get_err()),
                        float(last_base_ang_vel_obs[0]), float(last_base_ang_vel_obs[1]), float(last_base_ang_vel_obs[2]),
                        float(last_projected_gravity_obs[0]), float(last_projected_gravity_obs[1]), float(last_projected_gravity_obs[2]),
                        float(last_desired_contacts[i // 2]),
                    ])
                if loop_count % max(1, args.log_csv_every * 100) == 0:
                    log_csv_file.flush()

            if loop_count % 200 == 0:
                tau_max = max(abs(motors[name].Get_tau()) for name in active_joint_names)
                print(
                    f"stage={diag_stage_name} cmd=[{args.command_x:+.3f}, {args.command_y:+.3f}, {args.command_yaw:+.3f}] phase={last_phase:.3f} contacts={fmt_vec(last_desired_contacts, 0)} "
